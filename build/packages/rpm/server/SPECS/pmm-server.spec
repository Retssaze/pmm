...
License:    AGPLv3
URL:        https://%{provider}
Source:    https://%{provider}/archive/%{commit}/%{repo}-%{shortcommit}.tar.gz
...
# in a future, if there would be service for pmm-server - podman package should conflict with that
%package    podman-systemd
Version:    %{version}
Release:    %{rpm_release}
...
# pmm-server-podman-systemd package assign version
sed -i -e "s/PMM_TAG_VERSION/%{version}/" build/packages/config/pmm-server-podman.service

%install

# pmm-server-podman-systemd package
install -D -m 0644 build/packages/config/pmm-server-podman.service %{buildroot}/%{_unitdir}/pmm-server.service
install -D -p -m 0644 build/packages/config/pmm-server-podman.env %{buildroot}/%{_sysconfdir}/%{repo}/pmm-server.env
...
%preun podman-systemd
%systemd_preun pmm-server.service

%postun podman-systemd
...
* Thu Aug 18 2022 Denys Kondratenko <denys.kondratenko@percona.com> - 2.31.0-1
